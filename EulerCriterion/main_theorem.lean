...
lemma natDegree_frob_poly_eq :
  ...
  have hf : frob_poly K ≠ 0 := by
    simpa [frob_poly] using (X_pow_card_sub_X_ne_zero (K := K))
...
lemma roots_of_frob_poly (a : K) :
  ...
  · simp only [IsRoot.def, eval_sub, eval_pow, eval_X]
    ...
    simp only [sub_self]
  · exact X_pow_card_sub_X_ne_zero (K := K)
...
lemma card_imgK_finset :
  ...
  simpa [imgK_finset] using
    (Finset.card_image_of_injective (s := (Finset.univ : Finset K)) hinj)
...
lemma imgK_finset_subset_roots :
  ...
  exact Multiset.mem_toFinset.mpr (roots_of_frob_poly (K := K) a)
...
lemma card_roots_lower_bound :
  ...
  have hsubset := imgK_finset_subset_roots (K := K)
  have := Finset.card_le_card hsubset
  simpa [card_imgK_finset (K := K)] using this
...
lemma card_roots_frob_poly :
  ...
  classical
  have hl := card_roots_lower_bound (K := K)
  have hu := card_roots_upper_bound (K := K)
  have hd := natDegree_frob_poly_eq (K := K)
  exact le_antisymm
...
lemma roots_toFinset_eq_imgK_finset :
  ...
  · exact imgK_finset_subset_roots (K := K)
  · rw [card_roots_frob_poly (K := K), card_imgK_finset (K := K)]
...
theorem image_criterion (b : AlgebraicClosure K) :
  ...
  · intro h
    ...
    have : b ∈ imgK_finset K := by
      ...
      rwa [roots_toFinset_eq_imgK_finset (K := K)] at this
...
theorem forward_direction (hchar : ringChar K ≠ 2) (a : K) :
  ...
  have : ((Fintype.card K - 1) / 2) * 2 = Fintype.card K - 1 := by
    ...
    have hoddpow : (ringChar K) ^ (n : ℕ) % 2 = 1 := by
      rw [Nat.pow_mod]
      rw [odd_char_of_finite_field (K := K) hchar]
      norm_num
  ...
  have hb' : b ^ (Fintype.card K) = b := by
    ...
    norm_num
  rw [← image_criterion (K := K) b] at hb'
  rcases hb' with ⟨c, hc⟩
  use c
...
theorem euler_criterion
  ...
  · intro h
    ...
    have hcharodd : (ringChar K) % 2 = 1 :=
      odd_char_of_finite_field (K := K) hchar
    have : (Fintype.card K - 1) / 2 * 2 = Fintype.card K - 1 := by
      rw [← Nat.dvd_iff_div_mul_eq]
      have hcharconversion : CharP K (ringChar K) := ringChar.charP K
      have : ∃ n : ℕ+, Nat.Prime (ringChar K) ∧ Fintype.card K = (ringChar K) ^ (n : ℕ) :=
    ...
    have hbnonzero : b ≠ 0 := by
      ...
      tauto
    exact power_eq_self (K := K) b (Fintype.card K) hbnonzero this
  · intro h
    have := forward_direction (K := K) hchar a h
    rcases this with ⟨b, hb⟩
    use b
    rw [hb]
    exact pow_two b
